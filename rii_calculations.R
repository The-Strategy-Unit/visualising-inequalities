...
model_df_all_variables <- activity_long |>
  group_by(metric_name) |>
  mutate(cumulative_proportion_pop = cumsum(proportion_pop)) |>
  mutate(adj_IMD_decile = case_when(
...
sii_results <- sii_staging |>
  ...
  mutate(
    siiLcl95 = sii + qnorm(p = (1 - confLevel) / 2, lower.tail = TRUE) * siiSe,
